...
public class MyCITS2200Project implements CITS2200Project {
	...
	*/
	public int getShortestPath(String urlFrom, String urlTo) {
		if(urlFrom.equals(urlTo)) return 0;
			...
			MyBFS bfs = new MyBFS(); //CLASS IMPLEMENTATION
	...
	*/
	public String[] getCenters() {
		GraphCenters gc = new GraphCenters(); //CLASS IMPLEMENTATION
	...
	*/
	public String[][] getStronglyConnectedComponents() {
		MySCC myscc = new MySCC(); //CLASS IMPLEMENTATION
		return myscc.getSCC();
	}
	...
	*/
	public String[] getHamiltonianPath() {
		DPHamiltonian hamImp = new DPHamiltonian(this.maxvd); //CLASS IMPLEMENTATION
	...
	public class GraphCenters {
		...
		**/
		public String[] getCenters() {
			...
			if(size <= 0) return new String[0]; //NO MINIMUM ECCENTRICITIES ARE DEFINED

			// -- FORMULATE RETURN ARRAY --
			String[] centers = new String[size];
			int len = minEcc.length(); //GREATEST VERTEX DESCRIPTOR IN SET OF MINIMUM ECCENTRICITIES 
			int j = 0; //MANAGE BIT INDEX
			int count = 0; //MANAGE ARRAY POSITIONING COUNT
			j = minEcc.nextSetBit(j);
			while(j<len && count<size) {
				centers[count++] = MyCITS2200Project.this.wikiLookup.get(j);
				minEcc.set(j, false); //REMOVE FROM SET -- AVOID DOUBLE ENTRY
